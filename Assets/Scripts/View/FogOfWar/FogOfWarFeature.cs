...
namespace View.FogOfWar
{
    ...
    ///
    /// All external RTs are imported via renderGraph.ImportTexture() so that every
    /// cmd.Blit operates on TextureHandle↔TextureHandle. This is required because
    /// cmd.Blit(Texture, TextureHandle) silently fails on DX12.
    /// </summary>
    public class FogOfWarFeature : ScriptableRendererFeature
    {
        ...
        class FogOfWarPass : ScriptableRenderPass
        {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                // Import external RTs into RenderGraph as TextureHandles.
                ...
                using (var builder = renderGraph.AddUnsafePass<PassData>("FogOfWar", out var passData))
                {
                    ...
                    builder.SetRenderFunc(static (PassData data, UnsafeGraphContext ctx) =>
                    {
                        ...
                        {
                            cmd.Blit(data.tempB, data.tempA, data.blurMat, 0); // H
                            cmd.Blit(data.tempA, data.tempB, data.blurMat, 1); // V
                        }
