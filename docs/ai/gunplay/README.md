...
- ~~**Battle HUD**~~ — ✅ SHIPPED 2026-05-26 (spec locked 2026-05-21). Procedural SDF, no UI artist. Shipped stages: status effects row + tooltips (UTK, TR corner) + universal worldspace status mini-icons (player + bots, under HP bar) + radial stamina ring + exhaustion-gate (hysteresis lockout, `StaminaConfig` migration) + hotbar weapon slots (click=equip/holster, drag=swap, ammo-preserving) + ammo counter (mag/reserve/type, low=gold/empty=red). Armor paper-doll CUT (read poorly at HUD scale). Bleed baseline + GodMode passthrough + material-instance fixes (no more `Crosshair.mat` git churn) shipped alongside. HP bar stays worldspace-only. See [`battle-hud.md`](battle-hud.md) for stage-by-stage log.
