...
> **Status**: Stage 1 SHIPPED + polished (2026-05-15). Tech foundation + 1:1 IMGUI port + ADS top arm cutoff + outline + EFD-style hit pulse (4 per-type profiles) + flame-gradient charge fill (4-arm mode) + overheat tremble + tunable charge curve shape (ChargeRatioPower + ChargeTimeOverride) + **per-archetype cursor** (Ballistic 4-arm, Laser segmented ring 12-slice clockwise fill). Toggle Y in Play for A/B with IMGUI. **Ready for Stage 2 (directional recoil kick)**.
