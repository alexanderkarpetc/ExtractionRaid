...
### Tier A — Modal (Weapon Builder, Inventory, Crafting)
...
Player-facing UI is fully UI Toolkit (Inventory, Hotbar, Builder, Tooltip,
Crafting, Dialogue, EndOfRaid). The only remaining uGUI consumer is
`QuestItemView` (reward slot prefab) — out-of-scope for player input
flows. If new uGUI surfaces appear that need to talk to UTK overlays
(tooltip, etc.), the coordinate systems differ:
...
Use `RuntimePanelUtils.ScreenToPanel(panel, new Vector2(x, Screen.height - y))`
to convert (flip Y first — uGUI is bottom-left, UTK is top-left). See
`Assets/Scripts/View/UI/UiPanelHitTest.cs` for the canonical pattern.

API pattern — give callers the entry point that fits their stack so neither
side has to know the math:

- `Show(model, screenPos)` — screen-pixel flavour. Conversion happens inside.
- `ShowFromPanel(model, panelPos)` — panel-coord flavour. No conversion.
