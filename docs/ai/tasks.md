...
Джерело: фідбек Льохи Нудного. Записи ще не пріоритизовані; пов’язані епіки вказані для
планування, але не замінюють ці конкретні критерії перевірки.
...
| PT2 | UX | Під час утримання кнопки використання аптечки показувати однозначний progress/стан застосування; додати підказку `Hold`, де це доречно. | M4.1 | 🔄 active |
| PT3 | Функціональність | Дозволити використовувати аптечки й бинти безпосередньо з інвентаря, а не лише зі швидкого доступу; поведінка та списання предмета збігаються в обох потоках. | M4.1, M4.2 | ⬜ queued |
| PT4 | Баг | `Advanced Medkit` можна перетягнути у валідний слот швидкого доступу й використати звідти. | M4.1, M4.2 | ⬜ queued |
...
| PT6 | Баг | За укриттям персонаж не відтворює hit reaction без фактичного влучання/шкоди; реальний hit feedback лишається синхронним із damage event. | M4.5 | ⬜ queued |
...
| PT11 | Візуальний баг | Броня PMC не рендериться окремо від бота, коли сам бот прихований системою видимості. | M2.4 | 🔄 active |
...
| PT17 | Dev-баг | Увімкнений God Mode захищає від гранат так само, як від інших джерел шкоди; додати regression-перевірку вибухової шкоди. | M4.5 | 🔄 active |
